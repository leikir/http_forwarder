...
RSpec.describe 'forward spec gem', type: :request do
  ...
  before do
    ...
    load Rails.root.join('config', 'initializers', 'forwarder.rb')

    stub_request(
      :post, 'http://doggy.woof/doggos'
    ).with(
      body: modified_body,
      headers: { 'Content-Type' => 'blabla/json', 'Accept' => 'blibli/json' }
    ).to_return(status: 200)

    stub_request(
      :put, 'http://doggos.woof/dogs/4'
    ).with(
      headers: { 'Accept' => 'blibli/json' }
  ...
  it 'forward with modification on request' do
    post '/dogs', params: body, headers: { 'Accept' => 'blibli/json', 'X-Band' => 'Pixies' }
    expect(response.status).to eq(200)
  end

  it 'forward with modification on response' do
    put '/dogs/4', params: body, headers: { 'Accept' => 'blibli/json', 'X-Band' => 'Pixies' }
